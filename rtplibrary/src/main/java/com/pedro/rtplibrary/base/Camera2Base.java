...
public abstract class Camera2Base implements GetAacData, GetVideoData, GetMicrophoneData {
    private static final String TAG = Camera2Base.class.getSimpleName();
    ...
    public Camera2Base(Context context, boolean useOpengl) {
        this(null, context, useOpengl);
    }

    public Camera2Base(SurfaceTexture surfaceTexture, Context context, boolean useOpengl) {
    ...
    public void startPreview(String cameraId, int width, int height, int rotation) {
        if (!isStreaming() && !onPreview && (!isBackground || surfaceTexture != null)) {
            previewWidth = width;
            ...
            if (surfaceView != null) {
                ...
                cameraManager.prepareCamera(surfaceTexture, videoEncoder.getFps());
            ...
            if (cameraId != null) {
                cameraManager.openCameraId(Integer.parseInt(cameraId));
            } else {
    ...
    public void startPreview(CameraHelper.Facing cameraFacing, int width, int height, int rotation) {
        ...
        try {
            cameraId = cameraManager.getCameraIdForFacing(cameraFacing);
        } catch (CameraAccessException e) {
            Log.e(TAG, "Error", e);
        }
        startPreview(cameraId, width, height, rotation);
    }
    ...
    public void startPreview(String cameraId) {
        startPreview(cameraId, videoEncoder.getWidth(), videoEncoder.getHeight(), CameraHelper.getCameraOrientation(context));
    }
